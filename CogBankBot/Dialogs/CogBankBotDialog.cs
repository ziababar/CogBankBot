...
//    }
 ...
 //               WineCategories = await new WineApi().GetWineCategoriesAsync();
            ...
            IDialogContext context, IAwaitable<IEnumerable<Attachment>> result)
        {
            ...
            //            byte[] imageBytes = await new WineApi().GetUserImageAsync(attachment.ContentUrl);
...
//            List[] wines = await new WineApi().SearchAsync(wineTypeID, Rating, InStock, SearchTerms);
